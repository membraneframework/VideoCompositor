...
export function useInputStreams() {
    ...
    const instanceCtx = useSyncExternalStore(ctx.globalInputStreamStore.subscribe, ctx.globalInputStreamStore.getSnapshot);
    return instanceCtx;
}
/**
 ...
 */
export function useAudioInput(inputId, audioOptions) {
    const ctx = useContext(LiveCompositorContext);
    useEffect(() => {
 ...
 * offline processing.
 */
export function useAfterTimestamp(timestamp) {
    const ctx = useContext(LiveCompositorContext);
 ...
 */
export function newBlockingTask(ctx) {
 ...
 */
export function useBlockingTask(fn) {
    const ctx = useContext(LiveCompositorContext);
